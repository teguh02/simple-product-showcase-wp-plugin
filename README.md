...
- **🔧 Changed**: Section `price` pada shortcode `[sps_detail_products]` sekarang menampilkan harga asli dan harga diskon
  - Jika ada harga diskon: Harga asli ditampilkan dengan strikethrough (garis coret) dan harga diskon ditampilkan di sampingnya
  - Harga asli menggunakan class `sps-product-price-original` dengan data attribute `data-price` untuk scraping
  - Harga diskon menggunakan class `sps-product-price-discount` dengan data attribute `data-price` untuk scraping
  ...
  - Files Changed:
    - `includes/class-sps-cpt.php`: Menambahkan field `price_discount` di meta box dan save logic
    - `includes/class-sps-shortcodes.php`: Modifikasi `render_product_price()` untuk menampilkan harga asli dan diskon
    - `simple-product-showcase.php`: Modifikasi case 'price' di fallback handler
  - Now: Admin dapat mengatur harga diskon untuk produk, dan tampilan akan menampilkan harga asli dengan strikethrough dan harga diskon dengan class selector yang jelas untuk scraping

### Version 1.6.23 (December 2025)
...
- **🔄 Refactor**: Menggunakan template card produk yang sama untuk kategori dan query search
  ...
  - Solution: Gunakan template yang sama (sps-products-grid, sps-product-item, sps-detail-button) untuk kedua kasus
  - Now: Satu template yang sama, hanya berbeda di cara mendapatkan produk (filter)
- **✨ Improvements**:
  - Template card produk konsisten untuk semua kasus
  - CSS untuk product card ditambahkan di shortcode products_sub_category
  - Menggunakan struktur HTML yang sama: sps-product-title-text dan sps-detail-button
  - Responsive grid layout yang konsisten
- **🔧 Technical**:
  - Update template untuk kategori: menggunakan sps-product-title-text dan sps-detail-button
  - Update template untuk query search: menggunakan sps-products-grid yang sama
  - Menambahkan CSS untuk product card di shortcode products_sub_category
  - Menghapus CSS khusus untuk sps-products-grid-search

### Version 1.6.14 (January 2025)
